...
namespace GhostPlugin.Commands
{
    ...
    public class SendBroadcast : ICommand
    {
        ...
        public bool Execute(ArraySegment<string> arguments, ICommandSender sender, out string response)
        {
            ...
            {
                response = "사용법: SendBroadcast <메시지> <색깔> <텍스트 사이즈> <지속 시간>";
                return false;
            }

            string message_Argumanet = arguments.At(0);
            string message_Color = arguments.At(1);
            string message_Size = arguments.At(2);
            string duration = arguments.At(3);
            
            string message = $"<size={message_Size}><color={message_Color}>{message_Argumanet}</color></size>";
            if (player == null)
            {
            ...
            response = $"메시지가 모든 유저에게 전송되었습니다: {message}";
            return true;
        }
