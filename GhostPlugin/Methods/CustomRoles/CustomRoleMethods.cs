            ...
            try
            {
                ...
                while (enumerator.MoveNext())
                {
                    ...
                    if (enumerator.Current is not null)
                    {
                        ...
                        {
                            Log.Debug(
                                $"Custom Roles: Validation check failed | {enumerator.Current.StartTeam} {enumerator.Current.Chance}% || {random}");
                            continue;
                        }

                        Log.Debug($"Custom Roles: Returning a role! | {enumerator.Current.StartTeam} {enumerator.Current.Chance}% || {random}");
                        return (CustomRole)enumerator.Current;
                    }
                }

                Log.Debug("Custom Roles: Cannot move next");
